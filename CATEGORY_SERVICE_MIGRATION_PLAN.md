...
This is the comprehensive migration plan for the Category Service, documenting the complete implementation of all category functionality from the old Pika system using the new Clean Architecture and translation service integration.
...
- **Repository**: Complete CRUD operations with soft delete, hierarchy management, and materialized paths
- **Service Layer**: Business logic with validation, caching, and hierarchy operations
- **Controllers**: Public, Admin, and Internal controllers with proper validation and error handling
...
- **Audit Tracking**: `createdBy` and `updatedBy` fields for user tracking
- **Caching**: Redis integration for performance optimization
- **Validation**: Comprehensive business rules and circular reference prevention

## Translation Service Integration Strategy
...
// Database Schema
model Category {
  nameKey: String      // "category.name.{uuid}"
  descriptionKey: String?  // "category.description.{uuid}"
}

// Translation Service Integration
const translationKey = `category.name.${categoryId}`
const translatedName = await translationService.translate(translationKey, userLanguage)
```
...
1. **Category Creation Flow**:
   ...
   async createCategory(data: CreateCategoryData): Promise<Category> {
     ...
     const category = await this.repository.create({
       ...
       nameKey,
       descriptionKey,
       slug: this.generateSlug(nameKey),
...
3. **Search with Translation**:
   ```typescript
   // Search can work on translation keys or resolved text
   async searchCategories(search: string, language: string): Promise<Category[]> {
...
CREATE TABLE catalog.categories (
  ...
  name_key VARCHAR(255) NOT NULL,           -- Translation key: "category.name.{uuid}"
  description_key VARCHAR(255),             -- Translation key: "category.description.{uuid}"
  ...
  level INTEGER DEFAULT 1,                 -- 1=root, 2+=subcategories
  path VARCHAR(1000) DEFAULT '',           -- Materialized path "id1.id2.id3"
  created_by UUID NOT NULL,                -- User audit
  updated_by UUID,                         -- User audit
...
CREATE INDEX idx_categories_active_deleted ON categories(is_active, deleted_at);
```
...
const translationService = new TranslationServiceClient()
const translatedName = await translationService.translate(category.nameKey, userLanguage)
```
...
export const CategoryQueryParams = SearchParams.extend({
  ...
  sortBy: CategorySortBy.default('sortOrder'), // camelCase: 'name', 'sortOrder', 'createdAt'
})

// Admin API - Additional management features
...
// Internal API - Minimal data for service communication
export const InternalCategoryData = z.object({
  id: UUID,
  nameKey: z.string(),
...
class CategoryRepository {
  ...
  // - Level tracking
  // - Slug generation from nameKey
  // - Soft delete support
  // - Dependency checking (prevents deletion with children)
...
class CategoryService {
  ...
  async updateCategorySortOrder(id: string, sortOrder: number): Promise<Category>
