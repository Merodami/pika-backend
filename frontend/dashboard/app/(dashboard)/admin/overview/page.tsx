...
export default function AdminOverviewPage() {
  ...
  const { data: recentBusinesses, isLoading: businessesLoading } = useQuery({
    ...
    queryFn: async (): Promise<RecentBusiness[]> => {
      const response = await adminAdapter.getBusinesses({ page: 1 })
  ...
  return (
    <div className="space-y-6">
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              ...
              suffix={
                metrics?.growth.businesses !== undefined ? (
                  <span
                    className={`text-sm ${metrics.growth.businesses >= 0 ? 'text-green-500' : 'text-red-500'}`}
                  >
                    {metrics.growth.businesses >= 0 ? (
                      <ArrowUpOutlined />
                    ) : (
                      <ArrowDownOutlined />
                    )}
                    {Math.abs(metrics.growth.businesses)}%
                  </span>
                ) : undefined
              }
            />
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={16}>
          <Card title="Recent Service Businesses" loading={businessesLoading}>
            <Table
              dataSource={recentBusinesses}
          ...
          <Card title="Quick Actions">
            <div className="space-y-3">
              <button className="w-full p-3 text-left border rounded-lg hover:bg-gray-50 transition-colors">
                <div className="font-medium">Review Pending Businesses</div>
                <div className="text-sm text-gray-500">
                  Verify new service business applications
                </div>
              </button>
