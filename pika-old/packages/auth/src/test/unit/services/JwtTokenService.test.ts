...
describe('JwtTokenService', () => {
  ...
  describe('Token Generation', () => {
    ...
    it('should handle different user roles', async () => {
      ...
      const providerTokens = await jwtService.generateTokens(providerUser)

      expect(adminTokens.accessToken).toBeTruthy()
      expect(providerTokens.accessToken).toBeTruthy()
      ...
      const providerPayload = await jwtService.verifyToken(
        providerTokens.accessToken,
        'access',
      )

      expect(adminPayload.payload?.role).toBe(UserRole.ADMIN)
      expect(providerPayload.payload?.role).toBe(UserRole.PROVIDER)
    })
  })
