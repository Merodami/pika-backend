...
describe('PasswordSecurityService', () => {
  ...
  describe('Secure Password Generation', () => {
    ...
    it('should use cryptographically secure randomness', () => {
      // Test that generated passwords don't follow predictable patterns
      const passwords = []

      for (let i = 0; i < 50; i++) {
        passwords.push(passwordService.generateSecurePassword(16))
      }

      // Check that no two passwords share too many characters in same positions
      for (let i = 0; i < passwords.length - 1; i++) {
        for (let j = i + 1; j < passwords.length; j++) {
          ...
          // Should not have more than 3 characters in same positions (very unlikely with crypto random)
          expect(samePositions).toBeLessThan(4)
        }
      }
    })
  })
