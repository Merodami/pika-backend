...
export class PostgresTestContainer {
  ...
  async start(): Promise<PrismaClient> {
    ...
    try {
      const imageName = 'postgis/postgis:17-3.4-alpine' // Or 'postgis/postgis:17-3.4'

      console.log(`ℹ️ [PostgresTestContainer] Using Docker image: ${imageName}`)

      // Start container
      // Consider using the same image as your docker-compose if init.sql has PostGIS specific commands:
      // e.g., new PostgreSqlContainer('postgis/postgis:17-3.4')
      this.container = await new PostgreSqlContainer(imageName) // Or 'postgis/postgis:17-3.4' or your preferred version
