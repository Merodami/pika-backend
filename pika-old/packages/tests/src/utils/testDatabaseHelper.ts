...
export async function createTestDatabase(
  ...
  // Create container with PostGIS support
  let container = new PostgreSqlContainer('postgis/postgis:17-3.4')
  ...
  try {
    ...
    if (process.env.DEBUG) {
      ...
      const tables = await prisma.$queryRaw`
        ...
        FROM information_schema.tables 
        WHERE table_schema IN ('users', 'marketplace', 'auth', 'payments', 'audit')
