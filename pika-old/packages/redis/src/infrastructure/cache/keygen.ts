...
/**
 * HTTP-specific key generator: extracts params, query, and Accept-Language header
 * from a FastifyRequest in args[0], then hashes that.
 */
export function httpRequestKeyGenerator(
  ...
  // Expect args[0] to be a Fastify-like request with params, query and headers
  const req = args[0] as {
  ...
  // Clone into a plain JS object to strip any Fastify prototype or metadata
  const params = req.params ? Object.assign({}, req.params) : {}
  const query = req.query ? Object.assign({}, req.query) : {}

  // Extract language preference - this comes from the languageNegotiation plugin
  // which processes the Accept-Language header
  ...
  // Stable stringify and hash - now includes language in the cache key
  const payload = stringify({
    ...params,
    ...query,
    language, // Include language in the cache key
  })
