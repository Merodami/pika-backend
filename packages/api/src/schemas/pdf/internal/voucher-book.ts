...
export const InternalVoucherBookResponse = openapi(
  withTimestamps({
    ...
    isProcessing: z.boolean().default(false).describe('Whether book is currently being processed'),
    processingJobId: z.string().optional().describe('Current processing job ID'),
    errorMessage: z.string().optional().describe('Last error message if any'),
    retryCount: z.number().int().nonnegative().default(0).describe('Number of generation retries'),
  }),
  {
...
export type InternalVoucherBookResponse = z.infer<typeof InternalVoucherBookResponse>
...
export const InternalGeneratePdfJobRequest = openapi(
  z.object({
    ...
    options: z.object({
      includePages: z.boolean().default(true).describe('Include content pages'),
      includeCover: z.boolean().default(true).describe('Include cover page'),
      includeBackCover: z.boolean().default(true).describe('Include back cover'),
      quality: z.enum(['low', 'medium', 'high']).default('medium').describe('PDF quality'),
      format: z.enum(['A5', 'A4']).default('A5').describe('PDF format'),
      language: z.enum(['es', 'en']).default('es').describe('Content language'),
...
export type InternalGeneratePdfJobRequest = z.infer<typeof InternalGeneratePdfJobRequest>
...
export const InternalGeneratePdfJobResponse = openapi(
  z.object({
    ...
    pageCount: z.number().int().positive().optional().describe('Number of pages generated'),
...
export type InternalGeneratePdfJobResponse = z.infer<typeof InternalGeneratePdfJobResponse>
    ...
    pages: z.array(
      z.object({
        ...
        placements: z.array(
          z.object({
            id: UUID,
            contentType: ContentType.describe('Content type'),
            position: z.number().int().min(1).max(8).describe('Position (1-8)'),
            size: AdSize.describe('Ad size'),
            spacesUsed: z.number().int().min(1).max(8).describe('Spaces used'),
            imageUrl: z.string().url().optional().describe('Image URL'),
            qrCodePayload: z.string().optional().describe('QR code payload'),
...
export type InternalBookWithRelationsResponse = z.infer<typeof InternalBookWithRelationsResponse>
...
export const InternalRateLimitStatusResponse = openapi(
  z.object({
    ...
    remainingRequests: z.number().int().nonnegative().describe('Remaining requests'),
...
export type InternalRateLimitStatusResponse = z.infer<typeof InternalRateLimitStatusResponse>