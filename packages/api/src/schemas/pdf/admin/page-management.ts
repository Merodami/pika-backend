...
export const AdminVoucherBookPageResponse = openapi(
  withTimestamps({
    ...
    spacesAvailable: z.number().int().min(0).max(8).describe('Number of spaces available'),
...
export type AdminVoucherBookPageResponse = z.infer<typeof AdminVoucherBookPageResponse>
...
export const AdminAdPlacementResponse = openapi(
  withTimestamps({
    ...
    contentType: ContentType.describe('Type of content'),
    position: z.number().int().min(1).max(8).describe('Position on the page (1-8)'),
    size: AdSize.describe('Size of the ad placement'),
    spacesUsed: z.number().int().min(1).max(8).describe('Number of spaces used'),
    imageUrl: z.string().url().optional().describe('Content image URL'),
    qrCodePayload: z.string().optional().describe('QR code payload for vouchers'),
    ...
    revenue: z.number().optional().describe('Revenue generated from this placement'),
    impressions: z.number().int().nonnegative().optional().describe('Number of impressions'),
    clicks: z.number().int().nonnegative().optional().describe('Number of clicks'),
  }),
  {
...
export const UpdateAdPlacementRequest = openapi(
  z.object({
    contentType: ContentType.optional().describe('Type of content'),
    position: z.number().int().min(1).max(8).optional().describe('Position on the page (1-8)'),
    size: AdSize.optional().describe('Size of the ad placement'),
    imageUrl: z.string().url().optional().describe('Content image URL'),
    qrCodePayload: z.string().optional().describe('QR code payload for vouchers'),
    shortCode: z.string().max(20).optional().describe('Human-readable code'),
    title: z.string().max(255).optional().describe('Content title'),
...
export const PageLayoutPreviewRequest = openapi(
  z.object({
    layoutType: PageLayoutType.describe('Layout type for preview'),
    placements: z.array(
      z.object({
        position: z.number().int().min(1).max(8).describe('Position on the page'),
        size: AdSize.describe('Size of the placement'),
        contentType: ContentType.describe('Type of content'),
...
export const PageLayoutPreviewResponse = openapi(
  z.object({
    ...
    layout: z.object({
      totalSpaces: z.number().int().min(1).max(8).describe('Total available spaces'),
      usedSpaces: z.number().int().min(0).max(8).describe('Used spaces'),
      availableSpaces: z.number().int().min(0).max(8).describe('Available spaces'),
...
export type PageLayoutPreviewResponse = z.infer<typeof PageLayoutPreviewResponse>
...
export type BulkPlacementOperationRequest = z.infer<typeof BulkPlacementOperationRequest>
...
export type BulkPlacementOperationResponse = z.infer<typeof BulkPlacementOperationResponse>