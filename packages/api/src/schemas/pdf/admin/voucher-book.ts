...
import { VoucherBookStatus, VoucherBookType, VoucherBookSortBy, SortOrder, PageLayoutType, AdSize, ContentType } from '../../../common/schemas/enums.js'
...
export const AdminVoucherBookResponse = openapi(
  withTimestamps({
    ...
    pageCount: z.number().int().nonnegative().describe('Actual number of pages with content'),
    totalPlacements: z.number().int().nonnegative().describe('Total number of ad placements'),
    distributionCount: z.number().int().nonnegative().describe('Number of distribution records'),
  }),
  {
...
export const AdminVoucherBookQueryParams = SearchParams.extend({
  ...
  hasContent: z.boolean().optional().describe('Filter books with/without content'),
  hasPdf: z.boolean().optional().describe('Filter books with/without generated PDF'),
...
export type AdminVoucherBookDetailResponse = z.infer<typeof AdminVoucherBookDetailResponse>
...
export const PublishVoucherBookRequest = openapi(
  z.object({
    publishedAt: z.string().datetime().optional().describe('Publication date (defaults to now)'),
    generatePdf: z.boolean().default(true).describe('Generate PDF during publication'),
  }),
  {
...
export type PublishVoucherBookRequest = z.infer<typeof PublishVoucherBookRequest>
...
export type ArchiveVoucherBookRequest = z.infer<typeof ArchiveVoucherBookRequest>
...
export const GeneratePdfRequest = openapi(
  z.object({
    force: z.boolean().default(false).describe('Force regeneration even if PDF already exists'),
    priority: z.enum(['low', 'normal', 'high']).default('normal').describe('Generation priority'),
  }),
  {
...
export const GeneratePdfResponse = openapi(
  z.object({
    ...
    status: z.enum(['queued', 'processing', 'completed', 'failed']).describe('Generation status'),
    message: z.string().describe('Status message'),
    estimatedCompletion: z.string().datetime().optional().describe('Estimated completion time'),
    pdfUrl: z.string().url().optional().describe('PDF URL if already completed'),
  }),
  {
...
export type BulkVoucherBookOperationRequest = z.infer<typeof BulkVoucherBookOperationRequest>
...
export const BulkVoucherBookOperationResponse = openapi(
  z.object({
    successful: z.number().int().nonnegative().describe('Number of successful operations'),
    failed: z.number().int().nonnegative().describe('Number of failed operations'),
    results: z.array(
      z.object({
...
export type BulkVoucherBookOperationResponse = z.infer<typeof BulkVoucherBookOperationResponse>