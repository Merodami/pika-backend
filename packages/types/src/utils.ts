...
/**
 * Maps any role value to UserRoleType enum
 * Used by all mappers to ensure consistent role mapping
 */
export function mapUserRole(role: any): UserRoleType {
...
/**
 * Maps any status value to UserStatusType enum
 * Used by all mappers to ensure consistent status mapping
 */
export function mapUserStatus(status: any): UserStatusType {
 ...
 */
export function mapRoleToPermissions(role: UserRoleType): string[] {
  switch (role) {
    case UserRole.ADMIN:
      return [
        // User management
