...
import { businessSortFieldMapper } from '@pika/api/public'
import type {
  BusinessIdParam,
...
export class BusinessController {
  ...
  async getAllBusinesses(
    ...
    try {
      const query = getValidatedQuery<PublicBusinessQueryParams>(req)
  ...
  async getBusinessById(
    ...
    try {
      ...
      const query = getValidatedQuery<GetBusinessByIdQuery>(req)
  ...
  async createMyBusiness(
    req: Request<{}, {}, CreateBusinessRequest>,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
    try {
  ...
  async updateMyBusiness(
    req: Request<{}, {}, UpdateBusinessRequest>,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
    try {
      ...
      if (context.role !== 'BUSINESS') {
        throw ErrorFactory.forbidden('Only business owners can update their business')
      }

      // Get user's business first
      const existingBusiness = await this.businessService.getBusinessByUserId(userId)

      const business = await this.businessService.updateBusiness(existingBusiness.id, req.body)
