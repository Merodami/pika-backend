/**
 * Integration tests for the Business Service API
 *
 * Tests all endpoints with a real PostgreSQL testcontainer using Supertest.
 * Adapted from the old provider service tests to work with translation keys.
 */
...
describe('Business API Integration Tests', () => {
  ...
  describe('GET /businesses', () => {
    ...
    it('should only show active businesses to public', async () => {
      ...
      expect(response.body.data.every((business: any) => business.active)).toBe(true)
    })
  ...
  describe('GET /businesses/:business_id', () => {
    it('should return a specific business by ID', async () => {
      ...
      expect(response.body.businessNameKey).toBe(testBusinesses[0].businessNameKey)
    })
    ...
    it('should not show inactive businesses to public', async () => {
      const { testBusinesses } = await seedTestBusinesses(testDb.prisma, { generateInactive: true })
  ...
  describe('PUT /businesses/me', () => {
    it('should update current user business', async () => {
      const { testBusinesses, testUsers } = await seedTestBusinesses(testDb.prisma)
      const updateData = {
  ...
  describe('Admin Business API', () => {
    describe('GET /admin/businesses', () => {
      it('should return all businesses including inactive for admin', async () => {
        ...
        const inactiveCount = response.body.data.filter((b: any) => !b.active).length
      ...
      it('should filter by active status for admin', async () => {
        ...
        expect(inactiveResponse.body.data.every((b: any) => !b.active)).toBe(false)
      })
