...
export interface IBusinessService {
  getAllBusinesses(params: BusinessSearchParams): Promise<PaginatedResult<BusinessDomain>>
  getBusinessById(id: string, includes?: ParsedIncludes): Promise<BusinessDomain>
  getBusinessByUserId(userId: string, includes?: ParsedIncludes): Promise<BusinessDomain>
  createBusiness(data: CreateBusinessRequest): Promise<BusinessDomain>
  updateBusiness(id: string, data: UpdateBusinessRequest): Promise<BusinessDomain>
...
export class BusinessService implements IBusinessService {
   ...
   */
  async getAllBusinesses(params: BusinessSearchParams): Promise<PaginatedResult<BusinessDomain>> {
    try {
   ...
   */
  async getBusinessById(id: string, includes?: ParsedIncludes): Promise<BusinessDomain> {
    try {
   ...
   */
  async getBusinessByUserId(userId: string, includes?: ParsedIncludes): Promise<BusinessDomain> {
    try {
      logger.debug(`Fetching business by user ID: ${userId}`)

      const business = await this.businessRepository.findByUserId(userId, includes)
  ...
  async createBusiness(data: CreateBusinessRequest): Promise<BusinessDomain> {
    try {
      ...
      const existingBusiness = await this.businessRepository.findByUserId(data.userId)
      if (existingBusiness) {
        throw ErrorFactory.conflictError('Business already exists for this user', {
   ...
   */
  async updateBusiness(id: string, data: UpdateBusinessRequest): Promise<BusinessDomain> {
    try {
      ...
      if (data.businessDescription !== undefined) {
        if (data.businessDescription && existingBusiness.businessDescriptionKey) {
  ...
  async verifyBusiness(id: string): Promise<BusinessDomain> {
    try {
      ...
      const business = await this.businessRepository.update(id, { verified: true })
  ...
  async deactivateBusiness(id: string): Promise<BusinessDomain> {
    try {
      ...
      const business = await this.businessRepository.update(id, { active: false })
   ...
   */
  async updateBusinessRating(id: string, rating: number): Promise<BusinessDomain> {
    try {
      ...
      const business = await this.businessRepository.update(id, { avgRating: rating })
