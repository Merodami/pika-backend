...
export class CategoryRepository implements ICategoryRepository {
  ...
  async findAll(
    ...
    try {
      ...
      throw ErrorFactory.databaseError('findAll', 'Failed to fetch categories', error)
    }
  }
  ...
  async findById(id: string): Promise<Category | null> {
    try {
      ...
      return CategoryMapper.fromDocument(CategoryMapper.fromPrismaCategory(category))
    } catch (error) {
      logger.error('Error in CategoryRepository.findById:', error)
      throw ErrorFactory.databaseError('findById', 'Failed to fetch category', error)
    }
  }
  ...
  async findByIds(ids: string[]): Promise<Category[]> {
    try {
      ...
      throw ErrorFactory.databaseError('findByIds', 'Failed to fetch categories', error)
    }
  }
  ...
  async create(data: CreateCategoryData): Promise<Category> {
    try {
      ...
      throw ErrorFactory.databaseError('create', 'Failed to create category', error)
    }
  }
    ...
    try {
      ...
      if (data.parentId !== undefined) {
        if (data.parentId) {
          ...
          updateData.path = parent.path ? `${parent.path}.${parent.id}` : parent.id
        } else {
      ...
      throw ErrorFactory.databaseError('update', 'Failed to update category', error)
    }
  }
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      if (childrenCount > 0) {
        throw ErrorFactory.validationError('Cannot delete category with children')
      }
      ...
      throw ErrorFactory.databaseError('delete', 'Failed to delete category', error)
    }
  }
  ...
  async exists(id: string): Promise<boolean> {
    try {
      ...
      throw ErrorFactory.databaseError('exists', 'Failed to check category existence', error)
    }
  }
  ...
  async getHierarchy(rootId?: string): Promise<Category[]> {
    try {
      ...
      if (rootId) {
        ...
        where.OR = [
          { id: rootId },
          { path: { startsWith: rootCategory.path ? `${rootCategory.path}.${rootId}` : rootId } },
        ]
      }
      ...
      const domainCategories = CategoryMapper.fromPrismaCategoryArray(categories)
      return CategoryMapper.buildHierarchy(domainCategories)
    } catch (error) {
      logger.error('Error in CategoryRepository.getHierarchy:', error)
      throw ErrorFactory.databaseError('getHierarchy', 'Failed to fetch category hierarchy', error)
    }
  }
  ...
  async getPath(id: string): Promise<Category[]> {
    try {
      ...
      throw ErrorFactory.databaseError('getPath', 'Failed to fetch category path', error)
    }
  }
