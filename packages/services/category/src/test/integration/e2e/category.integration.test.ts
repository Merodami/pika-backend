...
vi.mock('@pika/api', async () => {
  const actualApi = await vi.importActual<typeof import('@pika/api')>('@pika/api')
  return actualApi
})

// Force Vitest to use the actual implementation of '@pika/shared' for this test file.
vi.mock('@pika/shared', async () => {
  const actualShared = await vi.importActual<typeof import('@pika/shared')>('@pika/shared')
  return actualShared
})
...
import { MockCacheService, createE2EAuthHelper, E2EAuthHelper } from '@pika/tests'
...
describe('Category API Integration Tests', () => {
  ...
  describe('GET /categories/hierarchy', () => {
    it('should return hierarchical category tree', async () => {
      ...
      const parentCategory = response.body.data.find((cat: any) => cat.level === 1)
