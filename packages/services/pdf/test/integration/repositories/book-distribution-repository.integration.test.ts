...
import { createTestDatabase, cleanupTestDatabase, clearTestDatabase } from '@pika/tests'
...
describe('BookDistributionRepository Integration Tests', () => {
  ...
  describe('findById', () => {
    ...
    it('should include relations when requested', async () => {
      const result = await repository.findById(createdDistribution.id, { book: true })
  ...
  describe('findAll', () => {
      ...
      for (const dist of distributions) {
        await testDb.prisma.bookDistribution.create({
          data: {
            ...
            createdBy: testUserId,
            updatedBy: testUserId,
            shippedAt: dist.status === 'shipped' || dist.status === 'delivered' ? new Date() : null,
    ...
    it('should filter by distributionType', async () => {
      ...
      expect(result.data).toHaveLength(2)
      expect(result.data.every(dist => dist.distributionType === 'initial')).toBe(true)
    })

    it('should sort results', async () => {
      const resultAsc = await repository.findAll({ sortBy: 'businessName', sortOrder: 'asc' })
      const resultDesc = await repository.findAll({ sortBy: 'businessName', sortOrder: 'desc' })
    ...
    it('should sort by quantity', async () => {
      const result = await repository.findAll({ sortBy: 'quantity', sortOrder: 'desc' })
  ...
  describe('update', () => {
    ...
    it('should update distribution properties', async () => {
      ...
      const result = await repository.update(distributionToUpdate.id, updateData)
    ...
    it('should throw error for non-existent distribution', async () => {
      await expect(
        repository.update(uuid(), { contactName: 'Updated', updatedBy: testUserId })
  ...
  describe('updateStatus', () => {
    ...
    it('should update status', async () => {
      const result = await repository.updateStatus(distribution.id, 'shipped', testUserId)
  ...
  describe('markAsDelivered', () => {
    ...
    it('should mark distribution as delivered', async () => {
      const result = await repository.markAsDelivered(shippedDistribution.id, testUserId)
  ...
  describe('getDistributionStats', () => {
    beforeEach(async () => {
      ...
      const distributions = [
        { status: 'pending', quantity: 100, distributionType: 'initial', businessId: uuid() },
        { status: 'shipped', quantity: 200, distributionType: 'reorder', businessId: uuid() },
        { status: 'delivered', quantity: 150, distributionType: 'initial', businessId: uuid() },
        { status: 'cancelled', quantity: 50, distributionType: 'replacement', businessId: uuid() },
        { status: 'delivered', quantity: 300, distributionType: 'initial', businessId: uuid() },
      ]
