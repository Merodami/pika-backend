...
import type { BookDistribution, DistributionStatus, BusinessType, DistributionType } from '@prisma/client'

import type { IBookDistributionRepository, IVoucherBookRepository } from '../repositories/index.js'
...
export interface IBookDistributionService {
  createDistribution(data: CreateBookDistributionData): Promise<BookDistribution>
  getDistributionById(id: string): Promise<BookDistribution>
  getAllDistributions(params: BookDistributionSearchParams): Promise<PaginatedResult<BookDistribution>>
  getDistributionsByVoucherBookId(voucherBookId: string): Promise<BookDistribution[]>
  updateDistribution(id: string, data: UpdateBookDistributionData): Promise<BookDistribution>
  deleteDistribution(id: string): Promise<void>
  shipDistribution(id: string, shippingData: {
    shippedQuantity: number
    trackingNumber?: string
    carrier?: string
    notes?: string
  ...
  confirmDelivery(id: string, deliveryData: {
    deliveryConfirmedBy?: string
    notes?: string
    updatedById: string
  }): Promise<BookDistribution>
...
export class BookDistributionService implements IBookDistributionService {
  ...
  async createDistribution(data: CreateBookDistributionData): Promise<BookDistribution> {
    try {
  ...
  async getAllDistributions(params: BookDistributionSearchParams): Promise<PaginatedResult<BookDistribution>> {
    try {
  ...
  async getDistributionsByVoucherBookId(voucherBookId: string): Promise<BookDistribution[]> {
    try {
      if (!isUuidV4(voucherBookId)) {
        throw ErrorFactory.badRequest('Invalid voucher book ID format')
      }

      const distributions = await this.distributionRepository.findByVoucherBookId(voucherBookId)
      return distributions
    } catch (error) {
      logger.error('Failed to get distributions by voucher book ID', { error, voucherBookId })
      throw ErrorFactory.fromError(error)
    }
  }

  async updateDistribution(id: string, data: UpdateBookDistributionData): Promise<BookDistribution> {
    try {
      if (!isUuidV4(id)) {
        throw ErrorFactory.badRequest('Invalid distribution ID format')
      }
      ...
      const updatedDistribution = await this.distributionRepository.update(id, data)

      // Invalidate cache
      await this.cache.del(`service:book-distribution:${id}`)
      await this.invalidateRelatedCache(currentDistribution.voucherBookId)

      logger.info('Distribution updated', { id, updatedFields: Object.keys(data) })
  ...
  async deleteDistribution(id: string): Promise<void> {
    try {
      ...
      if (distribution.status !== 'PENDING') {
        throw ErrorFactory.badRequest('Only pending distributions can be deleted')
      }
  ...
  async shipDistribution(id: string, shippingData: {
    shippedQuantity: number
    trackingNumber?: string
    carrier?: string
    notes?: string
    ...
    try {
      ...
      if (distribution.status !== 'PENDING') {
        throw ErrorFactory.badRequest('Only pending distributions can be shipped')
      }
      ...
      if (shippingData.shippedQuantity > distribution.requestedQuantity) {
        throw ErrorFactory.badRequest('Shipped quantity cannot exceed requested quantity')
      }
  ...
  async confirmDelivery(id: string, deliveryData: {
    deliveryConfirmedBy?: string
    notes?: string
    updatedById: string
  }): Promise<BookDistribution> {
    try {
      ...
      if (distribution.status !== 'SHIPPED') {
        throw ErrorFactory.badRequest('Only shipped distributions can be marked as delivered')
      }
  ...
  private async validateVoucherBookForDistribution(voucherBookId: string): Promise<void> {
    if (!isUuidV4(voucherBookId)) {
      throw ErrorFactory.badRequest('Invalid voucher book ID format')
    }
    ...
    if (voucherBook.status !== 'PUBLISHED') {
      throw ErrorFactory.badRequest('Only published voucher books can be distributed')
    }
  ...
  private validateStatusTransition(currentStatus: DistributionStatus, newStatus: DistributionStatus): void {
  ...
  private validateShippingData(data: UpdateBookDistributionData, currentDistribution: BookDistribution): void {
    if (data.shippedQuantity && data.shippedQuantity <= 0) {
      throw ErrorFactory.badRequest('Shipped quantity must be greater than 0')
    }

    if (data.shippedQuantity && data.shippedQuantity > currentDistribution.requestedQuantity) {
      throw ErrorFactory.badRequest('Shipped quantity cannot exceed requested quantity')
    }
  }
