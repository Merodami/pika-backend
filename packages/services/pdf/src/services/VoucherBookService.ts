...
import type { VoucherBook, VoucherBookStatus, BookFormat, Orientation } from '@prisma/client'
...
import { PDFGenerationService, GeneratePDFOptions, VoucherData } from './PDFGenerationService.js'
import { PageLayoutEngine, PageLayout, AdPlacementInfo } from './PageLayoutEngine.js'
...
export interface IVoucherBookService {
  ...
  getAllVoucherBooks(params: VoucherBookSearchParams): Promise<PaginatedResult<VoucherBook>>
  updateVoucherBook(id: string, data: UpdateVoucherBookData): Promise<VoucherBook>
...
export class VoucherBookService implements IVoucherBookService {
  ...
  async createVoucherBook(data: CreateVoucherBookData): Promise<VoucherBook> {
    try {
      logger.info('Creating voucher book', { title: data.title, createdById: data.createdById })

      // Validate and set defaults
      const bookData = {
        ...data,
        totalPages: data.totalPages || VoucherBookService.DEFAULT_PAGES,
        vouchersPerPage: data.vouchersPerPage || VoucherBookService.DEFAULT_VOUCHERS_PER_PAGE,
      ...
      await this.createInitialPages(voucherBook.id, bookData.totalPages, data.createdById)
  ...
  async getAllVoucherBooks(params: VoucherBookSearchParams): Promise<PaginatedResult<VoucherBook>> {
    try {
  ...
  async updateVoucherBook(id: string, data: UpdateVoucherBookData): Promise<VoucherBook> {
    try {
      if (!isUuidV4(id)) {
        throw ErrorFactory.badRequest('Invalid voucher book ID format')
      }
      ...
      if (data.totalPages || data.vouchersPerPage) {
        ...
        const vouchersPerPage = data.vouchersPerPage || currentBook.vouchersPerPage
      ...
      const updatedBook = await this.voucherBookRepository.update(id, updateData)

      // Invalidate cache
      await this.cache.del(`service:voucher-book:${id}`)

      logger.info('Voucher book updated', { id, updatedFields: Object.keys(data) })
      return updatedBook
    } catch (error) {
  ...
  async publishVoucherBook(id: string, userId: string): Promise<VoucherBook> {
    try {
      ...
      if (voucherBook.status !== 'READY_FOR_PRINT') {
        throw ErrorFactory.badRequest('Only books ready for print can be published')
      }
  ...
  async generatePDF(id: string, userId?: string): Promise<GeneratePDFResult> {
    ...
    try {
      ...
      const pageLayouts = await this.buildPageLayouts(voucherBook.totalPages, placements)
      ...
      for (const voucherId of voucherIds) {
        ...
        const payload = await this.cryptoServiceAdapter.generateVoucherPayload(voucherId)
      ...
      const pdfBuffer = await this.pdfGenerationService.generateVoucherBookPDF(pdfOptions)
  ...
  async getVoucherBookStatistics(id: string): Promise<{
    ...
    try {
      ...
      const totalSpaces = voucherBook.totalPages * VoucherBookService.SPACES_PER_PAGE
      
      // Calculate used spaces using layout engine
      let usedSpaces = 0
      for (const placement of placements) {
        if (placement.isActive) {
          const spacesRequired = this.pageLayoutEngine.getRequiredSpaces(placement.position as any)
      ...
      const placementsByType = placements.reduce((acc, placement) => {
        if (placement.isActive) {
  ...
  private async validateBookReadyForPublication(id: string): Promise<void> {
    ...
    if (placements.length === 0) {
      throw ErrorFactory.badRequest('Book must have at least one placement before publishing')
    }
    ...
    for (const placement of placements) {
      ...
      if (placement.contentType === 'IMAGE' && !placement.imageUrl) {
        throw ErrorFactory.badRequest(`Placement "${placement.title}" missing required image URL`)
      }

      if (placement.contentType === 'TEXT' && !placement.textContent) {
        throw ErrorFactory.badRequest(`Placement "${placement.title}" missing required text content`)
      }
    }
  }

  private async buildPageLayouts(totalPages: number, placements: any[]): Promise<PageLayout[]> {
    const pageLayouts: PageLayout[] = []

    for (let pageNumber = 1; pageNumber <= totalPages; pageNumber++) {
      const pagePlacements = placements.filter(p => 
        p.isActive && this.getPageNumberFromPosition(p.position) === pageNumber
      )
  ...
  private isSpaceOccupiedByPlacement(space: number, placement: AdPlacementInfo): boolean {
