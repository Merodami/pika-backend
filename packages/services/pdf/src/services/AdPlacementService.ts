...
export interface IAdPlacementService {
  ...
  updateAdPlacement(id: string, data: UpdateAdPlacementData): Promise<AdPlacement>
  deleteAdPlacement(id: string): Promise<void>
  reorderPlacements(voucherBookId: string, reorderData: ReorderPlacementData[], userId: string): Promise<void>
  validatePlacement(voucherBookId: string, position: AdPosition, excludeId?: string): Promise<PlacementValidation>
  getOptimalPlacementSuggestions(voucherBookId: string, contentType: ContentType): Promise<AdPosition[]>
}
...
export class AdPlacementService implements IAdPlacementService {
  ...
  async createAdPlacement(data: CreateAdPlacementData): Promise<AdPlacement> {
    try {
      ...
      const validation = await this.validatePlacement(data.voucherBookId, data.position)
      if (!validation.isValid) {
        throw ErrorFactory.badRequest(`Placement validation failed: ${validation.errors.join(', ')}`)
      }

      // Set display order if not provided
      const displayOrder = data.displayOrder ?? await this.getNextDisplayOrder(data.voucherBookId)
  ...
  async getAdPlacementsByVoucherBookId(voucherBookId: string): Promise<AdPlacement[]> {
    try {
      if (!isUuidV4(voucherBookId)) {
        throw ErrorFactory.badRequest('Invalid voucher book ID format')
      }

      const placements = await this.placementRepository.findByVoucherBookId(voucherBookId)
      return placements
    } catch (error) {
      logger.error('Failed to get ad placements by voucher book ID', { error, voucherBookId })
      throw ErrorFactory.fromError(error)
    }
  }

  async updateAdPlacement(id: string, data: UpdateAdPlacementData): Promise<AdPlacement> {
    try {
      if (!isUuidV4(id)) {
        throw ErrorFactory.badRequest('Invalid ad placement ID format')
      }
      ...
      if (data.position && data.position !== currentPlacement.position) {
        ...
        if (!validation.isValid) {
          throw ErrorFactory.badRequest(`Position validation failed: ${validation.errors.join(', ')}`)
        }
      }
      ...
      logger.info('Ad placement updated', { id, updatedFields: Object.keys(data) })
  ...
  async reorderPlacements(
    ...
    try {
      ...
      const placements = await this.getAdPlacementsByVoucherBookId(voucherBookId)
      const placementIds = new Set(placements.map(p => p.id))
      
      for (const { id } of reorderData) {
        if (!placementIds.has(id)) {
          throw ErrorFactory.badRequest(`Placement ${id} does not belong to voucher book ${voucherBookId}`)
        }
      }

      await this.placementRepository.reorderPlacements(voucherBookId, reorderData, userId)

      // Invalidate cache
      await this.invalidateRelatedCache(voucherBookId)

      logger.info('Ad placements reordered', { voucherBookId, count: reorderData.length })
    } catch (error) {
      logger.error('Failed to reorder ad placements', { error, voucherBookId, reorderData })
      throw ErrorFactory.fromError(error)
    }
  }
  ...
  async validatePlacement(
    ...
    try {
      ...
      const existingPlacements = await this.getAdPlacementsByVoucherBookId(voucherBookId)
      const activePlacements = existingPlacements.filter(p => 
        p.isActive && (!excludeId || p.id !== excludeId)
      )
      ...
      if (activePlacements.length === 0 && position !== 'FULL') {
        warnings.push('Consider using FULL size for maximum impact on empty page')
      }
      ...
      logger.error('Failed to validate placement', { error, voucherBookId, position })
      return {
        isValid: false,
  ...
  async getOptimalPlacementSuggestions(
    ...
    try {
      ...
      const positionsToTest: AdPosition[] = ['SINGLE', 'QUARTER', 'HALF', 'FULL']
      ...
      return this.prioritizeSuggestions(suggestions, contentType, existingPlacements.length)
    } catch (error) {
      logger.error('Failed to get optimal placement suggestions', { error, voucherBookId, contentType })
      throw ErrorFactory.fromError(error)
    }
  }
  ...
  private async validateVoucherBookModifiable(voucherBookId: string): Promise<void> {
    ...
    if (voucherBook.status === 'PUBLISHED' || voucherBook.status === 'ARCHIVED') {
  ...
  private validateContentTypeData(data: Partial<CreateAdPlacementData>): void {
    switch (data.contentType) {
      case 'IMAGE':
        if (!data.imageUrl) {
          throw ErrorFactory.badRequest('Image URL is required for IMAGE content type')
        }
        break
      case 'TEXT':
        if (!data.textContent) {
          throw ErrorFactory.badRequest('Text content is required for TEXT content type')
        }
        break
      case 'VOUCHER':
  ...
  private async getNextDisplayOrder(voucherBookId: string): Promise<number> {
    const existingPlacements = await this.getAdPlacementsByVoucherBookId(voucherBookId)
    const maxOrder = Math.max(...existingPlacements.map(p => p.displayOrder), 0)
    return maxOrder + 1
  }
  ...
  private buildOccupiedSpacesFromPlacements(placements: AdPlacement[]): Set<number> {
  ...
  private checkPositionConflict(position1: AdPosition, position2: AdPosition): boolean {
  ...
  private prioritizeSuggestions(
    ...
    const preferredOrder = priority[contentType] || ['SINGLE', 'QUARTER', 'HALF', 'FULL']
