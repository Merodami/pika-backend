...
export async function createPDFServer(config: ServerConfig) {
  ...
  // Initialize services
  const voucherBookService = new VoucherBookService(voucherBookRepository, cacheService)
  const adminVoucherBookService = new AdminVoucherBookService(voucherBookRepository, cacheService)

  // Initialize controllers
  const voucherBookController = new VoucherBookController(voucherBookService)
  const adminVoucherBookController = new AdminVoucherBookController(adminVoucherBookService)

  // Mount route handlers
  app.use('/voucher-books', createVoucherBookRoutes(voucherBookController))
  app.use('/admin/voucher-books', createAdminVoucherBookRoutes(adminVoucherBookController))
