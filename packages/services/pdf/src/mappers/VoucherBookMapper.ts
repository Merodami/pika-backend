import type { VoucherBook, VoucherBookStatus, VoucherBookType } from '@prisma/client'
...
export class VoucherBookMapper {
  ...
  static getDisplayPeriod(voucherBook: VoucherBookDTO): string {
    if (voucherBook.month) {
      const monthName = new Date(2000, voucherBook.month - 1, 1).toLocaleString('en', {
        month: 'long',
      })
  ...
  static toDTO(
    voucherBook: VoucherBook & {
      ...
      updatedByUser?: { id: string; email: string; firstName: string; lastName: string } | null
    ...
    if (voucherBook.pages || voucherBook.distributions) {
      const pagesWithPlacements = voucherBook.pages?.filter(
        page => page.adPlacements && page.adPlacements.length > 0
      ).length || 0

      const completionPercentage = voucherBook.totalPages > 0
      ...
      dto.statistics = {
        ...
        pendingDistributions: voucherBook.distributions?.filter(d => d.status === 'pending').length || 0,
        shippedDistributions: voucherBook.distributions?.filter(d => d.status === 'shipped').length || 0,
        deliveredDistributions: voucherBook.distributions?.filter(d => d.status === 'delivered').length || 0,
      }
    }
   ...
   */
  static groupByStatus(voucherBooks: VoucherBook[]): Map<VoucherBookStatus, VoucherBookDTO[]> {
    const statusMap = new Map<VoucherBookStatus, VoucherBookDTO[]>()
    
    // Initialize with all statuses
    const statuses: VoucherBookStatus[] = ['DRAFT', 'READY_FOR_PRINT', 'PUBLISHED', 'ARCHIVED']
   ...
   */
  static toGeneratePDFResponse(result: {
    voucherBook: any
    pdfUrl: string
    generatedAt: Date
    pageCount: number
  ...
  static toStatisticsResponse(stats: {
    total: number
    byStatus: { draft: number; readyForPrint: number; published: number; archived: number }
    byType: { monthly: number; specialEdition: number; regional: number }
    distributions: { total: number; pending: number; shipped: number; delivered: number }
    recentActivity: { booksCreatedThisMonth: number; pdfsGeneratedThisMonth: number; distributionsThisMonth: number }
  }): {
    total: number
    byStatus: { draft: number; readyForPrint: number; published: number; archived: number }
    byType: { monthly: number; specialEdition: number; regional: number }
   ...
   */
  static toAdminListResponse(result: {
    data: any[]
    pagination: any
  }): {
