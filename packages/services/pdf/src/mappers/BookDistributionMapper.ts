...
export class BookDistributionMapper {
  ...
  static toDTO(
    distribution: BookDistribution & {
      book?: { id: string; title: string; edition?: string | null; status: string } | null
   ...
   */
  static fromCreateDTO(dto: CreateBookDistributionDTO, createdById: string): any {
   ...
   */
  static fromUpdateDTO(dto: UpdateBookDistributionDTO, updatedById: string): any {
    const updates: any = {
    ...
    if (dto.deliveryAddress !== undefined) updates.deliveryAddress = dto.deliveryAddress
    ...
    if (dto.trackingNumber !== undefined) updates.trackingNumber = dto.trackingNumber
    if (dto.shippingCarrier !== undefined) updates.shippingCarrier = dto.shippingCarrier
   ...
   */
  static groupByStatus(distributions: BookDistribution[]): Map<string, BookDistributionDTO[]> {
  ...
  static getDaysInStatus(distribution: BookDistributionDTO): number {
    ...
    switch (distribution.status) {
      case this.STATUSES.SHIPPED:
        statusDate = distribution.shippedAt ? new Date(distribution.shippedAt) : new Date(distribution.createdAt)
        break
      case this.STATUSES.DELIVERED:
        statusDate = distribution.deliveredAt ? new Date(distribution.deliveredAt) : new Date(distribution.createdAt)
        break
      default:
