...
export interface IAdPlacementRepository {
  ...
  findById(id: string, parsedIncludes?: ParsedIncludes): Promise<AdPlacementDomain | null>
  findByPageId(pageId: string, parsedIncludes?: ParsedIncludes): Promise<AdPlacementDomain[]>
  findAll(params: AdPlacementSearchParams): Promise<PaginatedResult<AdPlacementDomain>>
  update(id: string, data: UpdateAdPlacementInput): Promise<AdPlacementDomain>
  delete(id: string): Promise<void>
  findByBookId(bookId: string, parsedIncludes?: ParsedIncludes): Promise<AdPlacementDomain[]>
  findByPosition(pageId: string, position: number): Promise<AdPlacementDomain | null>
  deleteByPageId(pageId: string): Promise<void>
  deleteByBookId(bookId: string): Promise<void>
  bulkUpdate(placementIds: string[], data: BulkUpdateAdPlacementInput): Promise<AdPlacementDomain[]>
  validatePlacementPosition(pageId: string, position: number, size: string, excludeId?: string): Promise<boolean>
}
...
export class AdPlacementRepository implements IAdPlacementRepository {
  ...
  async create(data: CreateAdPlacementInput): Promise<AdPlacementDomain> {
    try {
      logger.info('Creating ad placement', { pageId: data.pageId, position: data.position })
      ...
      if (!isValidPosition) {
        throw ErrorFactory.conflictError('Position is already occupied or conflicts with existing placements')
      }
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          throw ErrorFactory.conflictError('Position is already occupied on this page')
        }
        if (error.code === 'P2003') {
   ...
   */
  async findById(id: string, parsedIncludes?: ParsedIncludes): Promise<AdPlacementDomain | null> {
    try {
   ...
   */
  async findByPageId(pageId: string, parsedIncludes?: ParsedIncludes): Promise<AdPlacementDomain[]> {
    try {
      const include = parsedIncludes && Object.keys(parsedIncludes).length > 0
        ? toPrismaInclude(parsedIncludes) as Prisma.AdPlacementInclude
        : undefined
   ...
   */
  async findAll(params: AdPlacementSearchParams): Promise<PaginatedResult<AdPlacementDomain>> {
    try {
      const {
        page = 1,
        limit = 20,
   ...
   * Update ad placement
   */
  async update(id: string, data: UpdateAdPlacementInput): Promise<AdPlacementDomain> {
    try {
   ...
   */
  async findByBookId(bookId: string, parsedIncludes?: ParsedIncludes): Promise<AdPlacementDomain[]> {
    try {
      const include = parsedIncludes && Object.keys(parsedIncludes).length > 0
        ? toPrismaInclude(parsedIncludes) as Prisma.AdPlacementInclude
        : undefined
   ...
   */
  async findByPosition(pageId: string, position: number): Promise<AdPlacementDomain | null> {
    try {
      const placement = await this.prisma.adPlacement.findUnique({
        where: {
          pageId_position: {
   ...
   */
  async bulkUpdate(placementIds: string[], data: BulkUpdateAdPlacementInput): Promise<AdPlacementDomain[]> {
    try {
  ...
  async validatePlacementPosition(
    ...
    try {
      ...
      logger.error('Error validating placement position', { error, pageId, position })
      return false
    }
  }
}