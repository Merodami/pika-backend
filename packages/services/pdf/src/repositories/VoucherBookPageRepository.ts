...
export interface IVoucherBookPageRepository {
  ...
  findById(id: string, parsedIncludes?: ParsedIncludes): Promise<VoucherBookPageDomain | null>
  findByBookId(bookId: string, parsedIncludes?: ParsedIncludes): Promise<VoucherBookPageDomain[]>
  findAll(params: VoucherBookPageSearchParams): Promise<PaginatedResult<VoucherBookPageDomain>>
  update(id: string, data: UpdateVoucherBookPageInput): Promise<VoucherBookPageDomain>
  delete(id: string): Promise<void>
  findByPageNumber(bookId: string, pageNumber: number, parsedIncludes?: ParsedIncludes): Promise<VoucherBookPageDomain | null>
...
export class VoucherBookPageRepository implements IVoucherBookPageRepository {
   ...
   */
  async create(data: CreateVoucherBookPageInput): Promise<VoucherBookPageDomain> {
    try {
      logger.info('Creating voucher book page', { bookId: data.bookId, pageNumber: data.pageNumber })
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          throw ErrorFactory.conflictError('Page number already exists for this book')
        }
        if (error.code === 'P2003') {
   ...
   */
  async findById(id: string, parsedIncludes?: ParsedIncludes): Promise<VoucherBookPageDomain | null> {
    try {
   ...
   */
  async findByBookId(bookId: string, parsedIncludes?: ParsedIncludes): Promise<VoucherBookPageDomain[]> {
    try {
      const include = parsedIncludes && Object.keys(parsedIncludes).length > 0
        ? toPrismaInclude(parsedIncludes) as Prisma.VoucherBookPageInclude
        : undefined
   ...
   */
  async findAll(params: VoucherBookPageSearchParams): Promise<PaginatedResult<VoucherBookPageDomain>> {
    try {
      const {
        page = 1,
        limit = 20,
   ...
   */
  async update(id: string, data: UpdateVoucherBookPageInput): Promise<VoucherBookPageDomain> {
    try {
   ...
   */
  async findByPageNumber(bookId: string, pageNumber: number, parsedIncludes?: ParsedIncludes): Promise<VoucherBookPageDomain | null> {
    try {
      const include = parsedIncludes && Object.keys(parsedIncludes).length > 0
        ? toPrismaInclude(parsedIncludes) as Prisma.VoucherBookPageInclude
        : undefined
      ...
      throw ErrorFactory.databaseError('findByPageNumber', 'VoucherBookPage', error)
    }
  }
  ...
  async deleteByBookId(bookId: string): Promise<void> {
    try {
      ...
      throw ErrorFactory.databaseError('deleteByBookId', 'VoucherBookPage', error)
    }
  }
