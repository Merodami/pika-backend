...
export interface IBookDistributionRepository {
  ...
  findById(id: string, parsedIncludes?: ParsedIncludes): Promise<BookDistributionDomain | null>
  findByBookId(bookId: string, parsedIncludes?: ParsedIncludes): Promise<BookDistributionDomain[]>
  findAll(params: BookDistributionSearchParams): Promise<PaginatedResult<BookDistributionDomain>>
  update(id: string, data: UpdateBookDistributionInput): Promise<BookDistributionDomain>
  delete(id: string): Promise<void>
  findByBusiness(businessId: string, locationId?: string, parsedIncludes?: ParsedIncludes): Promise<BookDistributionDomain[]>
  updateStatus(id: string, status: string, updatedBy: string): Promise<BookDistributionDomain>
  markAsShipped(id: string, trackingNumber: string, carrier: string, updatedBy: string): Promise<BookDistributionDomain>
  deleteByBookId(bookId: string): Promise<void>
  getDistributionStats(bookId: string): Promise<DistributionStatsResult>
  markAsDelivered(id: string, deliveredBy: string): Promise<BookDistributionDomain>
}
...
export class BookDistributionRepository implements IBookDistributionRepository {
   ...
   */
  async create(data: CreateBookDistributionInput): Promise<BookDistributionDomain> {
    try {
   ...
   */
  async findById(id: string, parsedIncludes?: ParsedIncludes): Promise<BookDistributionDomain | null> {
    try {
   ...
   */
  async findByBookId(bookId: string, parsedIncludes?: ParsedIncludes): Promise<BookDistributionDomain[]> {
    try {
      const include = parsedIncludes && Object.keys(parsedIncludes).length > 0
        ? toPrismaInclude(parsedIncludes) as Prisma.BookDistributionInclude
        : undefined
      ...
      throw ErrorFactory.databaseError('findByBookId', 'BookDistribution', error)
    }
  }
   ...
   */
  async findAll(params: BookDistributionSearchParams): Promise<PaginatedResult<BookDistributionDomain>> {
    try {
      const {
        page = 1,
        limit = 20,
   ...
   */
  async update(id: string, data: UpdateBookDistributionInput): Promise<BookDistributionDomain> {
    try {
  ...
  async findByBusiness(
    ...
    try {
      ...
      throw ErrorFactory.databaseError('findByBusiness', 'BookDistribution', error)
    }
  }
   ...
   */
  async updateStatus(id: string, status: string, updatedBy: string): Promise<BookDistributionDomain> {
    try {
      const distribution = await this.prisma.bookDistribution.update({
        where: { id },
        data: {
      ...
      throw ErrorFactory.databaseError('updateStatus', 'BookDistribution', error)
    }
  }
  ...
  async markAsShipped(
    ...
    try {
      ...
      throw ErrorFactory.databaseError('markAsShipped', 'BookDistribution', error)
    }
  }
  ...
  async deleteByBookId(bookId: string): Promise<void> {
    try {
      ...
      throw ErrorFactory.databaseError('deleteByBookId', 'BookDistribution', error)
    }
  }
    ...
    try {
      ...
      statusCounts.forEach(item => {
        if (item.status in statusBreakdown) {
          statusBreakdown[item.status as keyof typeof statusBreakdown] = item._count._all
        }
      })
      ...
      quantityByStatus.forEach(item => {
        if (item.status in quantityByStatusMap) {
          quantityByStatusMap[item.status as keyof typeof quantityByStatusMap] = item._sum.quantity || 0
        }
      })
      ...
      throw ErrorFactory.databaseError('getDistributionStats', 'BookDistribution', error)
    }
  }
   ...
   */
  async markAsDelivered(id: string, deliveredBy: string): Promise<BookDistributionDomain> {
    try {
      const distribution = await this.prisma.bookDistribution.update({
        where: { id },
        data: {
      ...
      throw ErrorFactory.databaseError('markAsDelivered', 'BookDistribution', error)
    }
  }
}