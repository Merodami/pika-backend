...
export interface IVoucherBookRepository {
  ...
  findById(id: string, parsedIncludes?: ParsedIncludes): Promise<VoucherBookDomain | null>
  findAll(params: VoucherBookSearchParams): Promise<PaginatedResult<VoucherBookDomain>>
  update(id: string, data: UpdateVoucherBookInput): Promise<VoucherBookDomain>
  delete(id: string): Promise<void>
  findByYearAndMonth(year: number, month?: number, parsedIncludes?: ParsedIncludes): Promise<VoucherBookDomain[]>
  findPublishedBooks(params: PublishedBookSearchParams): Promise<PaginatedResult<VoucherBookDomain>>
  updateStatus(id: string, status: string, updatedBy: string): Promise<VoucherBookDomain>
...
export class VoucherBookRepository implements IVoucherBookRepository {
  ...
  async create(data: CreateVoucherBookInput): Promise<VoucherBookDomain> {
    try {
      logger.info('Creating voucher book', { title: data.title, year: data.year })
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          throw ErrorFactory.conflictError('Voucher book with this title and year already exists')
        }
      }
   ...
   */
  async findById(id: string, parsedIncludes?: ParsedIncludes): Promise<VoucherBookDomain | null> {
    try {
   ...
   */
  async findAll(params: VoucherBookSearchParams): Promise<PaginatedResult<VoucherBookDomain>> {
    try {
      const {
        page = 1,
        limit = 20,
   ...
   * Update voucher book
   */
  async update(id: string, data: UpdateVoucherBookInput): Promise<VoucherBookDomain> {
    try {
   ...
   */
  async findByYearAndMonth(year: number, month?: number, parsedIncludes?: ParsedIncludes): Promise<VoucherBookDomain[]> {
    try {
      ...
      throw ErrorFactory.databaseError('findByYearAndMonth', 'VoucherBook', error)
    }
  }
   ...
   */
  async findPublishedBooks(params: PublishedBookSearchParams): Promise<PaginatedResult<VoucherBookDomain>> {
    try {
      const {
        page = 1,
        limit = 20,
      ...
      throw ErrorFactory.databaseError('findPublishedBooks', 'VoucherBook', error)
    }
  }
   ...
   */
  async updateStatus(id: string, status: string, updatedBy: string): Promise<VoucherBookDomain> {
    try {
  ...
  async findBooksForGeneration(): Promise<VoucherBookDomain[]> {
    try {
      ...
      throw ErrorFactory.databaseError('findBooksForGeneration', 'VoucherBook', error)
    }
  }
}