...
describe('AdPlacementRepository Integration Tests', () => {
  ...
  describe('update', () => {
    it('should update ad placement content', async () => {
      ...
      expect(result.updatedAt.getTime()).toBeGreaterThan(adPlacement.updatedAt.getTime())
    })
  ...
  describe('reorderPlacements', () => {
    it('should reorder ad placements within a voucher book', async () => {
      ...
      await repository.reorderPlacements(testVoucherBook.id, reorderData, testUser.id)

      // Verify new order
      const reorderedPlacements = await repository.findByVoucherBookId(testVoucherBook.id)
