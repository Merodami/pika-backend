...
describe('VoucherBookRepository Integration Tests', () => {
  ...
  describe('create', () => {
    ...
    it('should prevent duplicate books with same title and year', async () => {
      ...
      await expect(repository.create(bookData)).rejects.toThrow('already exists')
    })
  ...
  describe('findAll', () => {
    beforeEach(async () => {
      // Create test books
      const books = [
        { title: 'January Book', bookType: 'MONTHLY', month: 1, year: 2024, status: 'PUBLISHED' },
        { title: 'February Book', bookType: 'MONTHLY', month: 2, year: 2024, status: 'DRAFT' },
        { title: 'March Book', bookType: 'MONTHLY', month: 3, year: 2024, status: 'PUBLISHED' },
        { title: 'Special Edition', bookType: 'SPECIAL', year: 2024, status: 'DRAFT' },
        { title: 'Annual Book', bookType: 'ANNUAL', year: 2023, status: 'PUBLISHED' },
      ]
    ...
    it('should filter by status', async () => {
      ...
      expect(result.data.every(book => book.status === 'PUBLISHED')).toBe(true)
    })
    ...
    it('should filter by bookType', async () => {
      ...
      expect(result.data).toHaveLength(3)
      expect(result.data.every(book => book.bookType === 'MONTHLY')).toBe(true)
    })
    ...
    it('should sort results', async () => {
      const resultAsc = await repository.findAll({ sortBy: 'title', sortOrder: 'asc' })
      const resultDesc = await repository.findAll({ sortBy: 'title', sortOrder: 'desc' })
  ...
  describe('updateStatus', () => {
    ...
    it('should update status to PUBLISHED and set publishedAt', async () => {
      const result = await repository.updateStatus(book.id, 'PUBLISHED', testUserId)
    ...
    it('should update status without setting publishedAt for non-published status', async () => {
      const result = await repository.updateStatus(book.id, 'READY_FOR_PRINT', testUserId)
  ...
  describe('findBooksForGeneration', () => {
    ...
    it('should find books ready for PDF generation', async () => {
      ...
      expect(result.every(book => book.status === 'READY_FOR_PRINT')).toBe(true)
