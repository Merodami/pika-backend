...
describe('PDF Voucher Book Integration Tests', () => {
  beforeAll(async () => {
    ...
    // 2. Update process.env for compatibility with existing code
    ...
    // 5. Initialize E2E Authentication Helper using the Express app
    authHelper = createE2EAuthHelper(app)

    // 6. Create test users and authenticate them
    logger.debug('Setting up E2E authentication...')
    await authHelper.createAllTestUsers(testDb.prisma)

    // 7. Get authenticated clients for different user types
  ...
  describe('Book Distribution', () => {
    ...
    describe('PUT /api/admin/pdf/distributions/:id/deliver', () => {
      ...
      it('should not deliver unshipped distribution', async () => {
        ...
        await adminClient
          .put(
            `/api/admin/pdf/distributions/${newDistResponse.body.data.id}/deliver`,
          )
          .expect(400)
      })
    })
      ...
      beforeEach(async () => {
        ...
        for (const dist of distributions) {
          ...
          if (dist.status === 'delivered') {
            await adminClient
              .put(
                `/api/admin/pdf/distributions/${response.body.data.id}/deliver`,
              )
              .expect(200)
          }
        }
  ...
  describe('Public API', () => {
    ...
    describe('GET /api/pdf/voucher-books', () => {
      ...
      it('should not require authentication', async () => {
        await unauthenticatedClient.get('/api/pdf/voucher-books').expect(200)
      })
