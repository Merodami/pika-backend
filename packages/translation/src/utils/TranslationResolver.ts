...
export class TranslationResolver {
  ...
  async resolve<T extends Record<string, any>>(
    ...
    try {
      ...
      const translations = await this.translationClient.getBulk(translationKeys, language)
    ...
    for (const keyField of keyFields) {
      ...
      if (typeof translationKey === 'string') {
        ...
        if (resolvedContent || options.fallbackToKey) {
          ...
          const value = resolvedContent || (options.fallbackToKey ? translationKey : '')
 ...
 */
export function createTranslationResolver(translationClient: TranslationClient): TranslationResolver {
